...
package object shapeless extends LowPriorityImplicits {
   ...
   */
  implicit def hconsOptionDCT[K <: Symbol, V, T <: HList](
    implicit
    witK: Witness.Aux[K],
    dbT: DebeziumPrimitiveType[V],
    dctT: DebeziumCompositeType[T]
  ): DebeziumCompositeType[FieldType[K, Option[V]] :: T] = new DebeziumCompositeType[FieldType[K, Option[V]] :: T] {
...
trait LowPriorityImplicits {
   ...
   */
  implicit def hconsDCT[K <: Symbol, V, T <: HList](
    implicit
    witK: Witness.Aux[K],
    dbT: DebeziumPrimitiveType[V],
    dctT: DebeziumCompositeType[T]
  ): DebeziumCompositeType[FieldType[K, V] :: T] = new DebeziumCompositeType[FieldType[K, V] :: T] {
    def schema: List[DebeziumFieldSchema] =
